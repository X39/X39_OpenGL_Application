/* tga.c
  ...
  ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    ...
    17 BYTE    Image Descriptor
               ...
               5),  the first line of image data is the top line
...
int load_TGA( tTGA *tga, const char *filename ) {
	...
	/* header info */
	if (fread( buffer, 1, SIZEOF_TGA_HEADER, f ) != SIZEOF_TGA_HEADER)
		return FALSE;
	...
	/* load image data */
	for (y = (is_inverted ? (image_height -1) : 0);(is_inverted ? (y >= 0) : (y < (int)image_height));(is_inverted ? (--y) : (++y)))
	{
		for (x = 0; x < image_width; x++)
		{
			...
			else
			{
				...
				if (image_pixel_size > 3)	data[ j +3 ] = buffer[ 3 ];
			}
		}
	}
	...
	tga->sizeOfDataArray =  raster_width * image_height;
...
//DUMMY FUNCTION!
//ToDo: Remove this function (is replaced by MaterialManagers loadMaterial function)
GLuint load_texture(const tTGA* tga, GLint wrap_s, GLint wrap_t ) {
